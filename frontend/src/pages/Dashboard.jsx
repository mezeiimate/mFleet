...
const Dashboard = () => {
  ...
  const activeVehs = vehicles.filter(v => v.status === 'Aktív').length;
  const serviceVehs = vehicles.filter(v => v.status === 'Szervizben').length;
  const alertCount = alerts.length; 

  const fuelStats = vehicles.reduce((acc, v) => { acc[v.fuel_type] = (acc[v.fuel_type] || 0) + 1; return acc; }, {});
  const catStats = vehicles.reduce((acc, v) => { acc[v.category] = (acc[v.category] || 0) + 1; return acc; }, {});
  ...
  return (
    <div className="space-y-8 pb-10 text-slate-800">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-gray-200 pb-6">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-slate-900">Áttekintés</h1>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
          <p className="text-gray-400 text-xs font-bold uppercase tracking-widest mb-1">Összes jármű</p>
          <div className="flex justify-between items-end"><span className="text-4xl font-black">{totalVehs}</span><div className="p-2 bg-slate-100 text-slate-600 rounded-lg"><Car size={24} /></div></div>
        </div>
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
          <p className="text-gray-400 text-xs font-bold uppercase tracking-widest mb-1">Aktív flottatag</p>
          <div className="flex justify-between items-end"><span className="text-4xl font-black text-green-600">{activeVehs}</span><div className="p-2 bg-green-50 text-green-600 rounded-lg"><CheckCircle size={24} /></div></div>
        </div>
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
          <p className="text-gray-400 text-xs font-bold uppercase tracking-widest mb-1">Szervizben</p>
          <div className="flex justify-between items-end"><span className="text-4xl font-black text-orange-500">{serviceVehs}</span><div className="p-2 bg-orange-50 text-orange-500 rounded-lg"><Wrench size={24} /></div></div>
        </div>
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
          <p className="text-gray-400 text-xs font-bold uppercase tracking-widest mb-1">Összes riasztás</p>
          <div className="flex justify-between items-end"><span className="text-4xl font-black text-red-600">{alertCount}</span><div className="p-2 bg-red-50 text-red-600 rounded-lg"><AlertTriangle size={24} /></div></div>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
          <div className="flex items-center gap-2 mb-6 border-b border-gray-50 pb-4"><Fuel className="text-blue-600" size={20} /><h2 className="font-bold text-lg">Üzemanyag típusok</h2></div>
          <div className="space-y-4">
            {Object.entries(fuelStats).map(([type, count]) => (
              <div key={type}>
                <div className="flex justify-between text-sm font-bold mb-1"><span>{type}</span><span className="text-slate-400">{Math.round((count/totalVehs)*100)}% ({count} db)</span></div>
                <div className="w-full bg-gray-100 h-2 rounded-full overflow-hidden"><div className="bg-blue-500 h-full rounded-full" style={{ width: `${(count/totalVehs)*100}%` }}></div></div>
              </div>
            ))}
          </div>
        </div>
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
          <div className="flex items-center gap-2 mb-6 border-b border-gray-50 pb-4"><LayoutGrid className="text-purple-600" size={20} /><h2 className="font-bold text-lg">Jármű kategóriák</h2></div>
          <div className="grid grid-cols-2 gap-4">
            {Object.entries(catStats).map(([cat, count]) => (
              <div key={cat} className="bg-slate-50 p-4 rounded-xl flex flex-col items-center justify-center"><span className="text-2xl font-black text-slate-800">{count}</span><span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{cat}</span></div>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-6 flex items-center gap-2 border-b border-gray-100"><AlertTriangle className="text-red-500" size={20} /><h2 className="font-bold text-lg text-slate-900">Sürgős teendők ({alerts.length} db)</h2></div>
        <div className="p-6 flex flex-col gap-3">
          {alerts.length === 0 ? (
            <div className="text-center py-10"><CheckCircle size={40} className="mx-auto text-green-400 mb-2" /><p className="text-gray-500 font-medium">Minden feladat elvégezve.</p></div>
          ) : (

...
const VehicleList = () => {
  ...
  // Új jármű alapállapot
  const [formData, setFormData] = useState({ id: null, license_plate: '', brand: '', model: '', year_of_manufacture: new Date().getFullYear(), vin: '', fuel_type: 'Benzin', transmission: 'Kézi', engine_capacity: '', current_km: 0, technical_exam_until: '', user_id: '', category: 'D1', status: 'Aktív' });
  ...
  const fetchData = () => {
    ...
    Promise.all([
      ...
      console.error("Kritikus hiba az adatbetöltéskor:", err);
      setLoading(false);
    });
  };
  ...
  const validateVehicleData = () => {
    ...
    if (!license_plate || !brand || !model || !year_of_manufacture || !vin || engine_capacity === '' || current_km === '' || current_km === null || !technical_exam_until) {
    ...
    if (!isOldFormat && !isNewFormat) {
      return "A rendszám formátuma érvénytelen (pl: ABC-123 vagy ABCD-123)!";
    }
    ...
    if (isNewFormat) {
      ...
      if (isFirstVowel !== isSecondVowel) {
        return "Az új típusú rendszám első két betűje csak két magánhangzó VAGY két mássalhangzó lehet!";
      }
    }

    const yearNum = Number(year_of_manufacture);
    if (yearNum < 1900 || yearNum > currentYear) {
      return `A gyártás évének 1900 és ${currentYear} között kell lennie!`;
    }

    if (vin.length !== 17) {
      return "Az alvázszámnak (VIN) pontosan 17 karakterből kell állnia!";
    }
    if (yearNum > 1981 && /[IOQ]/i.test(vin)) {
      return "1981 után gyártott járművek alvázszáma nem tartalmazhat 'I', 'O', vagy 'Q' betűket!";
    }
  ...
  const handleSaveVehicle = async (e) => {
    ...
    const error = validateVehicleData();
    if (error) {
      setFormError(error);
      return;
    }
    ...
    try {
      ...
      setFormError('Szerverhiba történt a mentés során.');
    }
  };
  ...
  const openAdd = () => { 
    ...
    setFormData({ id: null, license_plate: '', brand: '', model: '', year_of_manufacture: new Date().getFullYear(), vin: '', fuel_type: 'Benzin', transmission: 'Kézi', engine_capacity: '', current_km: 0, technical_exam_until: examDate, user_id: '', category: 'D1', status: 'Aktív' }); 
    setFormError('');
    setActiveModal('add'); 
  };
  ...
  const closeModal = () => { setActiveModal(null); setSelectedVehicle(null); setFormError(''); };

  // --- GOLYÓÁLLÓ SZŰRÉS ÉS RENDEZÉS ---
  ...
  return (
    <div className="space-y-6 relative">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button onClick={openAdd} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-all shadow-sm"><Plus size={20} /> Új jármű</button>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex flex-wrap gap-4 items-center" ref={filterRef}>
        ...
        <div className="relative flex-grow min-w-[200px]">
          ...
          <input type="text" placeholder="Rendszám, márka vagy modell..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm" />
        </div>
            ...
            <div className="absolute top-full mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-xl z-20 py-2">
              {['D1', 'D1m', 'D2', 'U'].map(cat => (
                <div key={cat} onClick={() => toggleSelection(selectedCategories, setSelectedCategories, cat)} className="flex items-center gap-3 px-4 py-2 hover:bg-gray-50 cursor-pointer">
                  {selectedCategories.includes(cat) ? <CheckSquare size={16} className="text-blue-600" /> : <Square size={16} className="text-gray-300" />} <span className="text-sm font-medium text-gray-700">{cat}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {openDropdown === 'driver' && (
            <div className="absolute top-full right-0 sm:left-0 mt-1 w-56 bg-white border border-gray-200 rounded-lg shadow-xl z-20 py-2 max-h-60 overflow-y-auto">
              <div onClick={() => toggleSelection(selectedDrivers, setSelectedDrivers, 'unassigned')} className="flex items-center gap-3 px-4 py-2 hover:bg-gray-50 cursor-pointer border-b border-gray-100">
                {selectedDrivers.includes('unassigned') ? <CheckSquare size={16} className="text-blue-600" /> : <Square size={16} className="text-gray-300" />} <span className="text-sm font-medium text-gray-500 italic">Nincs kiosztva</span>
              </div>
              {users.filter(Boolean).map(u => (
                <div key={u.id} onClick={() => toggleSelection(selectedDrivers, setSelectedDrivers, String(u.id))} className="flex items-center gap-3 px-4 py-2 hover:bg-gray-50 cursor-pointer">
                  {selectedDrivers.includes(String(u.id)) ? <CheckSquare size={16} className="text-blue-600" /> : <Square size={16} className="text-gray-300" />} <span className="text-sm font-medium text-gray-700">{u.name}</span>
                </div>
              ))}
            </div>
          )}
            ...
            <div className="absolute top-full right-0 sm:left-0 mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-xl z-20 py-2">
              {['Aktív', 'Szervizben'].map(status => (
                <div key={status} onClick={() => toggleSelection(selectedStatuses, setSelectedStatuses, status)} className="flex items-center gap-3 px-4 py-2 hover:bg-gray-50 cursor-pointer">
                  {selectedStatuses.includes(status) ? <CheckSquare size={16} className="text-blue-600" /> : <Square size={16} className="text-gray-300" />} <span className="text-sm font-medium text-gray-700">{status}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[900px] table-fixed">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="p-4 font-semibold text-gray-600 w-[15%]">Rendszám</th>
                <th className="p-4 font-semibold text-gray-600 w-[30%]">Jármű adatok</th>
                <th className="p-4 font-semibold text-gray-600 w-[20%]">Sofőr</th>
                <th className="p-4 font-semibold text-gray-600 text-center w-[15%]">Állapot</th>
                <th className="p-4 font-semibold text-gray-600 text-right w-[20%]">Műveletek</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                if (!v) return null;
                return (
                  <tr key={v.id || Math.random()} className={`hover:bg-blue-50 transition-colors ${v.status === 'Archivált' ? 'bg-gray-50 opacity-60 grayscale-[0.5]' : ''}`}>
                    ...
                    <td className="p-4">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => openDetails(v)} className="p-2 text-blue-500 hover:bg-blue-100 rounded-lg"><Info size={18} /></button>
                        <button onClick={() => openEdit(v)} className="p-2 text-gray-500 hover:bg-gray-200 rounded-lg"><Edit size={18} /></button>
                        {v.status === 'Aktív' && <button onClick={() => openError(v)} className="p-2 text-amber-500 hover:bg-amber-100 rounded-lg"><AlertTriangle size={18} /></button>}
                        {v.status !== 'Archivált' ? <button onClick={() => handleArchiveToggle(v)} className="p-2 text-slate-500 hover:bg-slate-200 rounded-lg"><Archive size={18} /></button> : <button onClick={() => handleArchiveToggle(v)} className="p-2 text-green-500 hover:bg-green-100 rounded-lg"><RefreshCw size={18} /></button>}
                      </div>
                    </td>
                  </tr>
                );
              ...
              <form onSubmit={handleSaveVehicle} className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input required maxLength={20} value={formData.license_plate} onChange={e => setFormData({...formData, license_plate: e.target.value.toUpperCase()})} className="w-full p-2 border rounded-lg font-mono placeholder:text-gray-300" placeholder="Pl: ABCD123 vagy ABC123" />
                  </div>
                  <div><label className="text-sm font-medium">Kategória*</label><select required value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full p-2 border rounded-lg bg-white"><option>D1</option><option>D1m</option><option>D2</option><option>U</option></select></div>
                  <div><label className="text-sm font-medium">Márka*</label><input required maxLength={50} value={formData.brand} onChange={e => setFormData({...formData, brand: e.target.value})} className="w-full p-2 border rounded-lg" /></div>
                  <div><label className="text-sm font-medium">Modell*</label><input required maxLength={50} value={formData.model} onChange={e => setFormData({...formData, model: e.target.value})} className="w-full p-2 border rounded-lg" /></div>
                  <div><label className="text-sm font-medium">Évjárat*</label><input required type="number" min="1900" max={new Date().getFullYear()} value={formData.year_of_manufacture} onChange={e => setFormData({...formData, year_of_manufacture: e.target.value === '' ? '' : parseInt(e.target.value)})} className="w-full p-2 border rounded-lg" /></div>
                  
                  <div>
                    <label className="text-sm font-medium">Alvázszám (VIN)*</label>
                    <input required minLength={17} maxLength={17} value={formData.vin} onChange={e => setFormData({...formData, vin: e.target.value.toUpperCase()})} className="w-full p-2 border rounded-lg uppercase font-mono" />
                  </div>
                  ...
                  <div>
                    ...
                    <select required value={formData.fuel_type} onChange={e => {
                      ...
                      setFormData({
                        ...formData, 
                        fuel_type: val, 
                        engine_capacity: val === 'Elektromos' ? 0 : (formData.engine_capacity === 0 ? '' : formData.engine_capacity)
                      });
                  ...
                  <div><label className="text-sm font-medium">Váltó*</label><select required value={formData.transmission} onChange={e => setFormData({...formData, transmission: e.target.value})} className="w-full p-2 border rounded-lg bg-white"><option>Kézi</option><option>Automata</option></select></div>
                  <div><label className="text-sm font-medium">Hengerűrtartalom (cm³)*</label><input required type="number" min="0" disabled={formData.fuel_type === 'Elektromos'} value={formData.engine_capacity} onChange={e => setFormData({...formData, engine_capacity: e.target.value === '' ? '' : parseInt(e.target.value)})} className="w-full p-2 border rounded-lg disabled:bg-gray-100 disabled:text-gray-500" /></div>
                  <div><label className="text-sm font-medium">Kilométeróra állás*</label><input required type="number" min="0" value={formData.current_km} onChange={e => setFormData({...formData, current_km: e.target.value === '' ? '' : parseInt(e.target.value)})} className="w-full p-2 border rounded-lg" /></div>
                  <div><label className="text-sm font-medium">Műszaki érvényessége*</label><input required type="date" value={formData.technical_exam_until} onChange={e => setFormData({...formData, technical_exam_until: e.target.value})} className="w-full p-2 border rounded-lg" /></div>
                  <div><label className="text-sm font-medium">Sofőr</label><select value={formData.user_id} onChange={e => setFormData({...formData, user_id: e.target.value})} className="w-full p-2 border rounded-lg bg-white"><option value="">Nincs kiosztva</option>{users.filter(Boolean).map(u => <option key={u.id} value={u.id}>{u.name}</option>)}</select></div>
                </div>
                <button type="submit" className="w-full bg-blue-600 text-white font-bold py-3 rounded-lg hover:bg-blue-700 mt-6 shadow-sm">Mentés</button>
              </form>
            )}
              ...
              <div className="p-6 space-y-6">
                <div className="bg-gray-50 p-6 rounded-xl border border-gray-200">
                  <div className="flex items-center justify-between mb-4 border-b border-gray-200 pb-4">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <span className="font-mono bg-white px-4 py-1.5 border rounded-lg text-xl font-bold">{selectedVehicle.license_plate || '-'}</span>
                    </div>
                  </div>
                  
                  {/* Bővített adatrács (NINCS ELEKTROMOS SZÖVEG) */}
                ...
                <div>
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <button onClick={openSticker} className="text-sm bg-blue-100 text-blue-700 px-3 py-1 rounded-lg hover:bg-blue-200 font-bold shadow-sm">+ Új matrica</button>
                  </div>
                    ...
                    <ul className="space-y-2 mb-6">
                      {activeStickers.map(vs => (
                        <li key={vs.id} className="flex justify-between items-center p-3 border border-gray-200 rounded-xl bg-white shadow-sm">
                          ...
                          <button onClick={() => handleDeleteSticker(vs.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Tévesen rögzített matrica törlése"><Trash2 size={18} /></button>
                        </li>
                      ))}
                    </ul>
                  )}
                      ...
                      <ul className="space-y-2 opacity-70">
                        {expiredStickers.map(vs => (
                          <li key={vs.id} className="flex justify-between items-center p-3 border border-gray-200 rounded-xl bg-gray-50">
                            ...
                            <button onClick={() => handleDeleteSticker(vs.id)} className="p-2 text-gray-400 hover:text-red-500 rounded-lg" title="Matrica eltávolítása a listából"><Trash2 size={18} /></button>
                          </li>
                        ))}
                      </ul>
                    </div>
              ...
              <form onSubmit={handleAddSticker} className="p-6 space-y-6">
                <div>
                  ...
                  <div className="flex gap-4">
                    <label className={`flex-1 flex flex-col items-center gap-2 p-4 rounded-xl border-2 cursor-pointer ${stickerTerritory === 'Országos' ? 'bg-blue-50 border-blue-500 text-blue-700' : 'border-gray-200'}`}>
                    ...
                    <label className={`flex-1 flex flex-col items-center gap-2 p-4 rounded-xl border-2 cursor-pointer ${stickerTerritory === 'Vármegyei' ? 'bg-blue-50 border-blue-500 text-blue-700' : 'border-gray-200'}`}>
                ...
                <div className="pt-4 border-t border-gray-100">
                  ...
                  <select required value={stickerData.sticker_type_id} onChange={handleStickerTypeChange} className="w-full p-3 border-2 border-gray-200 rounded-xl outline-none">
                ...
                <div className="grid grid-cols-2 gap-4 pt-2">
                  <div><label className="block text-xs font-bold mb-1 uppercase text-gray-500">Vételár (Ft)</label><input type="number" value={stickerData.purchase_price} onChange={e => setStickerData({...stickerData, purchase_price: parseInt(e.target.value) || 0})} className="w-full p-3 border border-gray-200 rounded-xl font-mono" /></div>
                  <div><label className="block text-xs font-bold mb-1 uppercase text-gray-500">Érvényesség vége*</label><input type="date" required value={stickerData.valid_until} onChange={e => setStickerData({...stickerData, valid_until: e.target.value})} className="w-full p-3 border border-gray-200 rounded-xl" /></div>
                </div>
                <div className="flex gap-3 mt-6 pt-6 border-t border-gray-100">
                  <button type="button" onClick={() => openDetails(selectedVehicle)} className="w-1/3 bg-gray-100 font-bold py-3 rounded-xl hover:bg-gray-200">Vissza</button>

  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button onClick={openAdd} className="flex items-center gap-2 bg-[#13395C] text-white px-4 py-2 rounded-lg hover:bg-[#0B2C4B] transition-all shadow-md">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex flex-wrap gap-4 items-center" ref={filterRef}>
        <div className="relative flex-grow min-w-[200px]">
          ...
          <input type="text" placeholder="Keresés név alapján..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#13395C] outline-none text-sm" />
        </div>
            ...
            <div className="absolute top-full mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-xl z-20 py-2">
              {['D1', 'D1m', 'D2', 'U'].map(cat => (
                <div key={cat} onClick={() => toggleSelection(selectedCategories, setSelectedCategories, cat)} className="flex items-center gap-3 px-4 py-2 hover:bg-gray-50 cursor-pointer">
                  {selectedCategories.includes(cat) ? <CheckSquare size={16} className="text-[#13395C]" /> : <Square size={16} className="text-gray-300" />} 
            ...
            <div className="absolute top-full mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-xl z-20 py-2">
              {['Országos', 'Vármegyei'].map(terr => (
                <div key={terr} onClick={() => toggleSelection(selectedTerritories, setSelectedTerritories, terr)} className="flex items-center gap-3 px-4 py-2 hover:bg-gray-50 cursor-pointer">
                  {selectedTerritories.includes(terr) ? <CheckSquare size={16} className="text-[#13395C]" /> : <Square size={16} className="text-gray-300" />} 
            ...
            <div className="absolute top-full right-0 sm:left-0 mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-xl z-20 py-2">
              {['Napi', 'Heti', 'Havi', 'Éves'].map(dur => (
                <div key={dur} onClick={() => toggleSelection(selectedDurations, setSelectedDurations, dur)} className="flex items-center gap-3 px-4 py-2 hover:bg-gray-50 cursor-pointer">
                  {selectedDurations.includes(dur) ? <CheckSquare size={16} className="text-[#13395C]" /> : <Square size={16} className="text-gray-300" />} 
                ...
                <tr key={st.id} className="hover:bg-[#D3D5D6]/10 transition-colors">
                  ...
                  <td className="p-4">
                    <div className="flex justify-end gap-2">
                      <button onClick={() => openEdit(st)} className="p-2 text-gray-500 hover:bg-gray-200 rounded-lg" title="Matrica adatainak módosítása"><Edit size={18} /></button>
                      <button onClick={() => handleDelete(st.id)} className="p-2 text-red-500 hover:bg-red-100 rounded-lg" title="Matrica típus törlése"><Trash2 size={18} /></button>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md animate-fade-in-up">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div>
                ...
                <input required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full p-3 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-[#13395C]" placeholder="Pl: D1 Éves Országos" />
                <p className="text-[10px] text-gray-500 mt-1">Érthető, egyértelmű megnevezés.</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Kategória*</label>
                  <select required value={formData.vehicle_category} onChange={e => setFormData({...formData, vehicle_category: e.target.value})} className="w-full p-3 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-[#13395C]">
                ...
                <div>
                  ...
                  <select required value={formData.duration} onChange={e => setFormData({...formData, duration: e.target.value})} className="w-full p-3 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-[#13395C]">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select required value={formData.territory} onChange={e => setFormData({...formData, territory: e.target.value})} className="w-full p-3 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-[#13395C]">
                ...
                <div>
                  ...
                  <input required type="number" min="0" value={formData.price} onChange={e => setFormData({...formData, price: parseInt(e.target.value) || ''})} className="w-full p-3 border border-gray-300 rounded-lg outline-none font-mono focus:ring-2 focus:ring-[#13395C]" />
                </div>
              </div>
              <div className="pt-4">
                <button type="submit" className="w-full py-3 bg-[#13395C] text-white font-bold rounded-xl hover:bg-[#0B2C4B] transition-colors shadow-sm">Mentés</button>
              </div>
            </form>
          </div>
        </div>

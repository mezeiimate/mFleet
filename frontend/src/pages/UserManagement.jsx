    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex flex-wrap gap-4 items-center" ref={filterRef}>
        <div className="relative flex-grow min-w-[200px]">
          ...
          <input type="text" placeholder="Keresés név vagy felhasználónév alapján..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#13395C] outline-none text-sm" />
        </div>
            ...
            <div className="absolute top-full mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-xl z-20 py-2">
              {[
                ...
                <div key={r.id} onClick={() => toggleRole(r.id)} className="flex items-center gap-3 px-4 py-2 hover:bg-gray-50 cursor-pointer">
                  {selectedRoles.includes(r.id) ? <CheckSquare size={16} className="text-[#13395C]" /> : <Square size={16} className="text-gray-300" />} 
                ...
                <tr key={user.id} className="hover:bg-[#D3D5D6]/10 transition-colors">
                  ...
                  <td className="p-4">
                    <div className="flex justify-end gap-2">
                      {/* HOVER TOOLTIPEK */}
                      <button onClick={() => openEdit(user)} className="p-2 text-gray-500 hover:bg-gray-200 rounded-lg" title="Adatok szerkesztése"><Edit size={18} /></button>
                      <button onClick={() => handleDelete(user.id)} className="p-2 text-red-500 hover:bg-red-100 rounded-lg" title="Felhasználó törlése"><Trash2 size={18} /></button>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md animate-fade-in-up">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div>
                ...
                <input required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full p-3 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-[#13395C]" />
                <p className="text-[10px] text-gray-500 mt-1">A dolgozó hivatalos neve (pl. Kovács János).</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Felhasználónév*</label>
                <input required value={formData.username} onChange={e => setFormData({...formData, username: e.target.value.toLowerCase()})} className="w-full p-3 border border-gray-300 rounded-lg outline-none font-mono focus:ring-2 focus:ring-[#13395C]" />
                <p className="text-[10px] text-gray-500 mt-1">Ékezet nélküli, egyedi azonosító (pl. jkovacs).</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Szerepkör*</label>
                <select required value={formData.role} onChange={e => setFormData({...formData, role: e.target.value})} className="w-full p-3 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-[#13395C]">
              ...
              <div>
                ...
                <input type="password" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} required={!formData.id} className="w-full p-3 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-[#13395C]" />
                <p className="text-[10px] text-gray-500 mt-1">Minimum 6 karakter ajánlott.</p>
              </div>
              <div className="pt-4">
                <button type="submit" className="w-full py-3 bg-[#13395C] text-white font-bold rounded-xl hover:bg-[#0B2C4B] shadow-sm transition-colors">Mentés</button>
              </div>
            </form>
          </div>
        </div>
